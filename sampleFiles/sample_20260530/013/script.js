...
class ThreeApp {
  ...
  async load() {
    ...
    // ローダーの loadAsync メソッドに読み込む画像のパスを指定
    const texture = await loader.loadAsync(imagePath);
    // 生成したテクスチャを map プロパティに設定
    this.material.map = texture;

    // - 画像とテクスチャ -----------------------------------------------------
    // loader という名前からもわかるとおり、three.js の各種ローダーはファイルな
    // どのリソースを「読み込み」するための機構です。
    // TextureLoader の場合は「画像などのリソースを読み込み、テクスチャを生成す
    // る」という一連の処理を行ってくれる一種のヘルパーのような役割を持っていま
    // す。なにが言いたいかというと、ローダーは常に「読み込み＋生成」の２つの異
    // なる処理を一括して行う機能を提供するということです。
    // 画像を読み込みするだけならもっと簡単な方法もありますが、それに加えて同時
    // にテクスチャというオブジェクトを生成するところまでをまとめてやってくれる、
    // ということがローダーのありがたい部分だということです。
    // ------------------------------------------------------------------------
  }
