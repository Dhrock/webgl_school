...
// = 019 ======================================================================
// いきなり 3D 数学はちょっと敷居が高いので、平面的な座標（2D の処理）をベースに
...
// が何を意味しているのかすぐに思い浮かべられるようにしておくと良いでしょう。ち
// なみに XY 平面と言った場合、X 軸と Y 軸のいずれにも水平な平面という意味になり
// ます。最初はちょっとイメージしにくいかもしれませんが、必要に応じて図解するな
// どして落ち着いて考えましょう。
// ============================================================================
...
class ThreeApp {
  ...
  constructor(wrapper) {
    ...
    window.addEventListener('pointermove', (pointerEvent) => {
      ...
      const scaleVector = new THREE.Vector2(scaleX, scaleY);

      // カメラ座標を考慮して、scaleX と scaleY の正負を計算して月の座標に割り当てる
      const cameraDirection = new THREE.Vector3(
        this.camera.position.x - ThreeApp.CAMERA_PARAM.lookAt.x,
        this.camera.position.y - ThreeApp.CAMERA_PARAM.lookAt.y,
        this.camera.position.z - ThreeApp.CAMERA_PARAM.lookAt.z,
      ).normalize();
      console.log('cameraDirection', cameraDirection);

      const theta_zx = Math.atan2(cameraDirection.z, cameraDirection.x);
      const thera_yz = Math.acos(cameraDirection.y);

      const Vec_zx = new THREE.Vector2(
  ...
  async load() {
    ...
    this.moonTexture = await loader.loadAsync(moonPath);
  }
